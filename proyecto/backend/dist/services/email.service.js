...
class EmailService {
    static async sendEmail({ to, subject, html }) {
        try {
            ...
            logger_1.default.info(`📧 Correo enviado exitosamente a ${to}`, {
                messageId: info.messageId,
                accepted: info.accepted,
            });
        }
        catch (error) {
            logger_1.default.error('❌ Error al enviar correo:', error);
    ...
    static async sendStudentCredentials(data) {
        const html = `
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
  <div class="container">
    ...
    static async sendProfessorCredentials(data) {
        const html = `
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <div class="container">
    ...
    static async verifyConnection() {
        try {
            ...
            logger_1.default.info('✅ Conexión al servidor SMTP verificada correctamente');
            return true;
        }
        catch (error) {
...
EmailService.transporter = nodemailer_1.default.createTransport({
    ...
    auth: {
        ...
        pass: process.env.SMTP_PASSWORD?.replace(/\s+/g, '') || process.env.SMTP_PASSWORD,
    },
});
